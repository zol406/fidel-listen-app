...
class _ResultsScreenState extends State<ResultsScreen> {
  ...
  Future<void> _performOcr() async {
    // This is a placeholder for path, because in cloud build we can't save/read from device
    // The history feature will not fully work in this build, but the OCR itself will.
    // In a real device build, widget.imagePath would be valid.
    final fakePath = widget.imagePath ?? '/fake/path/image.jpg';

    try {
      String text = await FlutterTesseractOcr.extractText(
        widget.imagePath!,
        language: 'amh',
        args: {"tessdata-dir-path": "/assets/tessdata/"},
      );
      
      final historyBox = Hive.box<ScanHistory>('scans');
      await historyBox.add(ScanHistory(
        imagePath: fakePath, // Use fake path for history
        recognizedText: text,
  ...
  Widget build(BuildContext context) {
    // Because we cannot access the file system in the cloud build,
    // we show a placeholder for the image instead of trying to read the file.
    final imageWidget = widget.imagePath != null && File(widget.imagePath!).existsSync()
      ? Image.file(File(widget.imagePath!), fit: BoxFit.contain)
      : Container(color: Colors.grey[300], child: const Center(child: Icon(Icons.image, size: 50, color: Colors.white)));
